...
import { Zap, TrendingUp, Info, Search, MapPin } from 'lucide-react';
import { getPrices, getItemIconUrl, getItemName, getEnchantedId } from '../lib/albion-api';
import { REFINE_RECIPES } from '../data/recipes';
import { ENCHANTMENTS, CITIES, MarketPrice } from '../types';

export function RefineScanner() {
  ...
  const [selectedRefine, setSelectedRefine] = useState<typeof REFINE_RECIPES[0] | null>(null);
  const [isPremium, setIsPremium] = useState(true);
  const [useFocus, setUseFocus] = useState(false);

  const filteredRefines = useMemo(() => {
    if (!searchTerm) return [];
    const lowerSearch = searchTerm.toLowerCase();
    return REFINE_RECIPES.filter(r => 
      r.name.toLowerCase().includes(lowerSearch) ||
      r.id.toLowerCase().includes(lowerSearch)
    );
  }, [searchTerm]);

  const analyzeRefine = async (recipe: typeof REFINE_RECIPES[0]) => {
    setLoading(true);
    setSelectedRefine(recipe);
  ...
  useEffect(() => {
    if (selectedRefine) {
      analyzeRefine(selectedRefine);
    }
  }, [selectedEnchantment, sellCity]);

  const sortedCities = useMemo(() => {
    if (!selectedRefine) return [];
    const productId = getEnchantedId(selectedRefine.id, selectedEnchantment);
  ...
  }, [allCityPrices, selectedRefine, selectedEnchantment]);

  const calculateProfit = () => {
    if (!selectedRefine) return null;
    
    const productId = getEnchantedId(selectedRefine.id, selectedEnchantment);
    const productPrice = prices[productId] || 0;
    
    // Refining RRR: 36.7% in bonus city, 15.2% elsewhere
    // If useFocus: 53.9% in bonus city, 43.5% elsewhere
    let rrr = 15.2;
    if (selectedRefine.bonusCity) {
        rrr = 36.7;
    }
    
    if (useFocus) {
        rrr = selectedRefine.bonusCity ? 53.9 : 43.5;
    }

    let rawCost = 0;
    selectedRefine.materials.forEach(m => {
  ...
  return (
    <div className="space-y-8">
      <div className="bg-[#1a1614] border border-[#2d2824] rounded-xl p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
          <h2 className="text-xl font-bold text-white italic flex items-center gap-2">
            ...
            Calculadora de Lucro de Refino
          </h2>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Pesquisar recurso para refinar (ex: Couro, Barra, Tecido...)"
        ...
        {searchTerm && filteredRefines.length > 0 && (
          <div className="absolute z-20 w-full mt-2 bg-[#1a1614] border border-[#2d2824] rounded-xl shadow-2xl max-h-[500px] overflow-y-auto overflow-x-hidden backdrop-blur-xl scrollbar-thin scrollbar-thumb-amber-500/20">
            {filteredRefines.map(recipe => (
              <button
                key={recipe.id}
                onClick={() => {
                  analyzeRefine(recipe);
                  setSearchTerm('');
                }}
      ...
      ) : result && selectedRefine ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div className="bg-[#1a1614] border border-[#2d2824] rounded-xl p-6">
              <h3 className="text-sm font-bold text-gray-500 uppercase mb-6 flex items-center gap-2">
                <Info size={14} /> Detalhes do Refino ({getItemName(getEnchantedId(selectedRefine.id, selectedEnchantment))})
              </h3>
              <div className="space-y-4">
                {selectedRefine.materials.map(m => {
                  const materialId = getEnchantedId(m.id, selectedEnchantment);
                  return (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedRefine.bonusCity && (
                <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-xl flex items-center gap-3">
                  <MapPin className="text-emerald-500" size={20} />
                  <div>
                    <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Melhor Local para Refinar</p>
                    <p className="text-sm font-bold text-white uppercase italic">{selectedRefine.bonusCity}</p>
                  </div>
                </div>
              )}
          ...
          <div className="bg-gradient-to-b from-[#1a1614] to-[#0d0b0a] border border-[#2d2824] rounded-2xl p-8 flex flex-col justify-center text-center h-fit">
            <img src={getItemIconUrl(getEnchantedId(selectedRefine.id, selectedEnchantment))} alt="" className="w-24 h-24 mx-auto mb-6" />
